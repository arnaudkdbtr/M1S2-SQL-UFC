...
Voici quelques exemples de requêtes montrant les capacités de la base de données :

### 1. Afficher le top 5 des combattants avec le plus de victoires
...
### 2. Trouver les combattants ayant la meilleure précision de frappe

```sql
SELECT 
    combattant_nom, 
    precision_frappes_pct 
FROM VUE_STATS_COMBATTANT
WHERE total_sig_frappes_tentees > 100
ORDER BY precision_frappes_pct DESC
LIMIT 10;
```

### 3. Analyser la distribution des méthodes de victoire par catégorie de poids
...
GROUP BY cat.weightclass, r.methode
ORDER BY cat.weightclass, nombre DESC;
```

### 4. Identifier les combattants avec le plus grand écart entre leur taille et celle de leurs adversaires

```sql
SELECT 
    c1.nom, 
    AVG(c1.taille_cm - c2.taille_cm) AS ecart_taille_moyen
FROM COMBAT cb
JOIN COMBATTANT c1 ON cb.combattant1_id = c1.id
JOIN COMBATTANT c2 ON cb.combattant2_id = c2.id
GROUP BY c1.nom
HAVING COUNT(*) >= 3
ORDER BY ecart_taille_moyen DESC;
```

### 5. Analyser l'avantage du combattant local dans les événements UFC

```sql
SELECT 
    CASE 
        WHEN INSTR(c.nom, SUBSTR(e.lieu, 1, INSTR(e.lieu, ',') - 1)) > 0 THEN 'Local'
        ELSE 'Visiteur'
    END AS statut,
    COUNT(*) AS nombre_combats,
    SUM(CASE WHEN r.vainqueur_id = c.id THEN 1 ELSE 0 END) AS victoires,
    ROUND(100.0 * SUM(CASE WHEN r.vainqueur_id = c.id THEN 1 ELSE 0 END) / COUNT(*), 2) AS pourcentage_victoire
FROM COMBAT cb
JOIN EVENEMENT e ON cb.evenement_id = e.id
JOIN COMBATTANT c ON cb.combattant1_id = c.id OR cb.combattant2_id = c.id
JOIN RESULTAT r ON cb.id = r.combat_id
GROUP BY statut;
```
